...
- BUILD実装済み(β): 機材パレット(既存11機種)→キャンバスクリックで配置、ドラッグ移動、
  ⚡接続(同ペア再クリックで撤去)、🗑撤去、設定は機器クリック(インスペクター)。diagnosticsが常時審判
- SAVE条件: 機材2台以上+ケーブル1本以上+**購読(パッチ)1つ以上**+ALL GREEN → 「自 カスタム」がSYSTEMに出現
- カスタムはCHALLENGE対応済み。TROUBLEは障害カタログ監査後に解禁予定 / MADI・AESは見送り
- src/buildmode.js に隔離実装(本体app.jsへの変更はフック4箇所のみ)
